...
getChar(istream& istr)
{
   ...
   switch (ch) {
      ...
      case ESC_KEY: {
         ...
         // Note: ARROW_KEY_INT == MOD_KEY_INT == HE_KEY_INT, so we only check MOD_KEY_INT
         if (combo == char(MOD_KEY_INT)) {
            ...
            else if ((key >= char(HE_KEY_BEGIN) && (key <= char(HE_KEY_END)))){
               return returnCh(int(key) + HE_KEY_FLAG);
            }
